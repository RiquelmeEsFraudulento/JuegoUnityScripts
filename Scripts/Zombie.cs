...
public class Zombie : MonoBehaviour
{
    ...
    public RuntimeAnimatorController animatorW;
    public RuntimeAnimatorController animatorR;
    ...
    void Start()
    {
      ...
      //animator = GetComponent<Animator>();
        ...
        if (userScore < 500)
        {
            ...
            animator.runtimeAnimatorController = animatorW;
            agentObject = CreateAgentPrefab(agentPrefab);
        }
        else
        {
            ...
            agentObject = CreateSpecialAgentPrefab();
    ...
    void Update()
    {
        ...
        if (isAlive)
        {
            ...
            //animator.SetBool("isRunning", isWalking && isAlive);
    ...
    {
        // Stop the agent's movement by setting its destination to its current position
        agent.SetDestination(transform.position);

        // Reset the agent's velocity
        agent.velocity = Vector3.zero;

        // Reset the agent's angular velocity
        agent.velocity = Vector3.zero;
    }
    ...
    GameObject CreateSpecialAgentPrefab()
    {
        ...
        GameObject specialAgentPrefabInstance = Instantiate(specialAgentPrefab, transform);
        ...
        attackRange = 4f + (float)(userScore / 25000);
        if (attackRange > 6f)
        {
            attackRange = 6f;
        }
